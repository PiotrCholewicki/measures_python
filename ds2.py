...
path2 = 'C:\\Users\\Piotr\\Desktop\\mieszkania\\skrypt_pp\\ds1.xlsx' #ZMIEN LOKALIZACJE
...
def mainloop():
    ...
    for floor in range(0, -1, -1):
        ...
        for room in range(50):
            ...
            # room_name = "POKÓJ " + str(floor_name) + str(room_index)
            room_name = "POKÓJ " + str(room_index)
            #add room sockets
            keyboard_input = input("POKÓJ " + room_name + ": \ninna nazwa lub STOP: ")
            keyboard_input = keyboard_input.upper()
            if(keyboard_input[0].isdigit()):
                sockets_count = int(keyboard_input)
                #add room name
                add_header(room_name)
                add_sockets("Gniazdko 1-fazowe", sockets_count)
                sockets_index = 1
                room_index += 1
                
            elif(keyboard_input == "STOP"):
                room = 36  # stop the loop
                wb2.save(path2)
                with open ('last_index.txt', 'w') as f:
                    f.write(str(global_index))
                break
                
            else: #for example: "WC DAMSKIE"
                room_name = keyboard_input
                room -= 1  # decrement room to not skip next room
                sockets_count = input("GNIAZDKA W " + room_name + ": ")
                while not sockets_count.isdigit():
                    sockets_count = input("ZŁA LICZBA GNIAZDEK, PODAJ JESZCZE RAZ: ")
                sockets_count = int(sockets_count)
                add_header(room_name)
                add_sockets("Gniazdko 1-fazowe", sockets_count)
                sockets_index = 1
